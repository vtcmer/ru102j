...
public class SiteGeoDaoRedisImpl implements SiteGeoDao {
    ...
    /**
     * Implemented with pipeline **bonus challenge**
     * @return
     */
    @Override
    public Set<Site> findAll() {
        try (Jedis jedis = jedisPool.getResource()) {
            ...
            List<Response<Map<String, String>>> data = new ArrayList<>();
            for (String key : keys) {
            ...
            pipeline.sync();
            if (!data.isEmpty()){
                for (Response<Map<String, String>>  item: data){
                    sites.add(new Site(item.get()));
                }
            }
